...
</html>`;var Xt=ct.z.string(),C=class{async handle(e,t,o){try{let r=Xt.parse(e.query.token);if(!r)return t.status(401).send(`${Oe}`);let a=new l,i=new fe(a),n=`Bearer ${r}`;return await i.execute(n)===!1?t.status(400).send(`${Oe}`):t.status(200).send(`${dt}`)}catch{return t.status(400).send("<h1>N\xE3o foi possivel validar seu e-mail</h1>")}}};var je=(0,lt.Router)(),Zt=new C;je.post("/verify-email",Zt.handle);var ft=g(require("cors")),gt=g(require("helmet"));var mt=require("express");var ge=class{constructor(e){this.showUserPerEmail=e}async execute(e){let t=await this.showUserPerEmail.show(e);if(!t)throw Error("User not found");await new u({userEmail:t.email,userFullName:t.fullName,userId:t.id,userPassword:t.password_hash}).sendEmailToForgotPassword()}};var we=class{async handle(e,t,o){try{let r=e.body.email;if(!r)return t.status(400).json({error:"Missing data"});let a=new S;return await new ge(a).execute(r),t.status(200).json({message:"Foi enviado um e-mail para redefinir sua senha"})}catch(r){o(r)}}};var ze=(0,mt.Router)(),Qt=new we;ze.post("/forgot-password",Qt.handle);var ht=require("express");var ye=class{constructor(e){this.showUserPerIdRepository=e}async execute(e,t){let o=new h(this.showUserPerIdRepository),r=`Bearer ${e}`,a=await o.validAuth(r);if(!a)throw Error("Invalid token");let i=await this.showUserPerIdRepository.show(a);if(!i)throw Error("User not found");let p=await new u({userEmail:i.email,userFullName:i.fullName,userId:i.id,userPassword:i.password_hash}).encryptedPassword(t);T.emit("user/reset-password",p,i.id)}};var ut=require("zod");var er=ut.z.string(),be=class{async handle(e,t,o){try{let r=er.parse(e.query.token),a=e.body.newPassword;if(!r)throw Error("Missing token to reset password");if(!a)throw Error("Missing data");let i=new l;return await new ye(i).execute(r,a),t.status(200).json({message:"Senha atualizada com sucesso"})}catch(r){o(r)}}};var qe=(0,ht.Router)(),tr=new be;qe.post("/reset-password",tr.handle);var Le=class{constructor(){this.app=(0,_e.default)(),this._middlewares(),this._router(),this._lastMiddlewares(),new he}_middlewares(){this.app.use(_e.default.json());let e="https://passtorage.vercel.app",t={origin:(o,r)=>{o===e?r(null,!0):r(new Error("Acesso n\xE3o permitido por CORS"))}};this.app.use((0,ft.default)(t)),this.app.use((0,gt.default)())}_lastMiddlewares(){this.app.use(nt)}_router(){this.app.use(N),this.app.use(U),this.app.use(Te),this.app.use(ve),this.app.use(Ce),this.app.use(je),this.app.use(ze),this.app.use(qe)}},rr=new Le;
